...
create function public.get_leaderboard(p_mode public.match_mode default null)
returns table (
  player_id uuid,
  display_name text,
  avatar_url text,
...
revoke all on function public.get_leaderboard(public.match_mode) from public;
grant execute on function public.get_leaderboard(public.match_mode) to authenticated;
